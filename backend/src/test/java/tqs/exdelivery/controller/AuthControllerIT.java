...
public class AuthControllerIT {
    ...
    void whenRegisterWithValidCredentials_thenReturnRegisteredUser() {
        given().when().header("Content-Type", "application/json").and().
            body(registerRequest).when().
            post(baseUrl + port + "/api/v1/register").
            then().statusCode(200).contentType(ContentType.JSON).
            and().body("email", is(registerRequest.getEmail())).
            and().body("password", not(registerRequest.getPassword())).
            and().body("name", is(registerRequest.getName()));
    }
    ...
    void whenLoginWithValidCredentials_thenReturnToken() {
        ...
        given().when().header("Content-Type", "application/json").and().
                body(validLoginRequest).when().
                post(baseUrl + port + "/api/v1/login").
                then().statusCode(200).contentType(ContentType.JSON).
                and().body("$", hasKey("accessToken")).
                and().body("tokenType", is("Bearer"));
    }
